...
python内置了读写文件的函数（open();write()），用法与c兼容

现代操作系统不允许普通的程序直接操作磁盘，所以，读写文件就是请求操作系统打开一个文件对象（文件描述符），然后，通过操作系统提供的接口（interface）从这个文件对象中读取数据（读文件），或者把数据写入这个文件对象（写文件 ）。
...
```python
>>> f = open('/Users/michael/test.txt', 'r') #‘r’，表示读
```

之后调用read()函数一次读取文件全部内容，read(size),表示一次性读取多少文件，readlines()表示每次读取一行。

读取二进制文件，将‘r’转换为‘rb’模式。
...
StringIO顾名思义是从内存中读取数据，要把str写入StringIO，我们需要先创建一个StringIO，然后，像文件一样写入即可：
...
StringIO操作的只能是str，如果操作二进制数据，就要使用BytesIO。
